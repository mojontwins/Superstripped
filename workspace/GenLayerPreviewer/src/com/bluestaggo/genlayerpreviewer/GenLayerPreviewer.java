...
public class GenLayerPreviewer {
	...
	private static final long seed = "404".hashCode(); // Set seed here
	...
	public static void main(String[] args) {
		...
		BiomeGenBase.buildBiomeStructure(WorldType.DEFAULT.getBiomesForWorldType());
		addGenLayers(GenLayer.initializeAllBiomeGenerators(seed, WorldType.DEFAULT)[0]);
		initWorldGenSeed();
	...
	private static GenLayer getLastGenLayer() {
		return genLayers.isEmpty() ? null : genLayers.get(genLayers.size() - 1);
	}
	...
	private static int getBiomeColor(int i) {
		if (i >= 0 && i < 256 && BiomeGenBase.biomeList[i] != null) {
			...
			if (biome.biomeColor == 0) {
				biome.biomeColor = random.nextInt(0xFFFFFF);
			}
	...
	private static class GenLayerArray extends GenLayer {
		...
		@Override
		public int[] getInts(int chunkX, int chunkZ, int areaWidth, int areaHeight) {
			return array;
		}
	}
}